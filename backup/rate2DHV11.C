...
void rate2DHV11(){
  ...
  file1.open("txtdir/me11alct.txt");
  file2.open("txtdir/me11clct.txt");
  file3.open("txtdir/me11tmb.txt");
  ...
  for (string line; getline(file2,line); ){
    ...
    for (int j=0;j<11;j++){
      iss >> clctrate[i][j];
      if(i<12) clctrate[i][j] = clctrate[i][j]*1.5;
    }
    i++;
  }
  i=0;
  for (string line; getline(file3,line); ){
    ...
    for (int j=0;j<11;j++){
      iss >> tmbrate[i][j];
      if(i<12) tmbrate[i][j] = tmbrate[i][j]*1.5;
    }
    i++;
  }
