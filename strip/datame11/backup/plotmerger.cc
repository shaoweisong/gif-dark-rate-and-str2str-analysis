...
TF1 * nfit(TGraphErrors * graph, double mean, int& status){
    ...
    ff->SetParLimits(0, 0.01*mean, 10*mean);
    ff->SetParameter(1,-100);
    ff->SetParLimits(1, -100000, 0);
    ff->SetParameter(2,-0.01);
    ff->SetParLimits(2, -100, 0);
    ff->SetParameter(3,-10);
    ff->SetParLimits(3, -100000, 0);
    ff->SetParameter(4,-0.001);
    ff->SetParLimits(4, -100, 0);
    ff->SetLineColor(4);
    status = graph->Fit(ff,"SREQ");
...
void plotmerger(const char * dirnameChar1, const char * dirnameChar2, const char * fnameTChar="ClosedCircuit_ShortCables_WithBox_0-300V_", const char * ext=".txt"){
   //setTDRStyle();
   ...
   TString dirname1 = TString(dirnameChar1);   
   TString dirname2 = TString(dirnameChar2);
   TString filename = TString(fnameTChar);
   ifstream myfile1;
   ifstream myfile2;
   ofstream ofile("output.txt");
   TString pdfFName = "output.pdf";
   dirname1+="/"; 
   dirname2+="/";
   TString suffix[12] = {"layer1_strips12", "layer1_strips23", "layer2_strips12", "layer2_strips23", "layer3_strips12", "layer3_strips23", "layer4_strips12", "layer4_strips23", "layer5_strips12", "layer5_strips23", "layer6_strips12", "layer6_strips23"};
    ...
    TCanvas * canv[6];
    std::cout << dirname1 << std::endl;
    std::cout << dirname2 << std::endl;
    ofile << n4mean-lastPoints << "\t" << frMin << "\t" << frMax << "\n";
    for(int j = 0; j < 1 ; j++) {
        int canvN = j/2; cout << "canvN::  " << canvN << endl;
        int padN  = 1;
        //if(canvN*2==j){
        ...
        //  padN=3;  
        //}
        myfile1.open(dirname1+filename+suffix[j]+TString(ext));
        myfile2.open(dirname2+filename+suffix[j]+TString(ext));
        ...
        std::vector<int> * vX[2]    = {&vecX1, &vecX2};
        std::vector<double> * vY[2] = {&vecY1, &vecY2};
        char str1[100];
        char str2[100];
        char str3[100];
        ...
        bool beginning = true;
        bool startSeria[2] = {true, true};
        int t0[2]          = {0,    0};
        if (myfile1.is_open()&&myfile2.is_open()){                        
            while (!myfile1.eof()){
                myfile1 >>str1>>str2>>str3>>voltage>> data_points;
                TString tmps1 = str2; tmps1+=" "; tmps1+=str3;
                int t1 = TDatime(tmps1.Data()).Convert();
                data_points*=1e12;//pA
                if (voltage==300){
                    setT0(startSeria[0], t0[0], t1);
                    vecX1.push_back(t1);
            ...
            while (!myfile2.eof()){
                myfile2 >>str1>>str2>>str3>>voltage>> data_points;
                TString tmps2 = str2; tmps2+=" "; tmps2+=str3;
                int t2 = TDatime(tmps2.Data()).Convert();
                data_points*=1e12;//pA
                if (voltage==300){
                    setT0(startSeria[1], t0[1], t2);
                    vecX2.push_back(t2);
            ...
            double mean[2] = {0,0};
            double rms[2]  = {0,0};     
            double xmax1 = vX[0]->at(vX[0]->size()-1);
            double xmax2 = vX[1]->at(vX[1]->size()-1);
            ...
            canv[canvN]->cd();
            dataGr[j][0]->SetMarkerStyle(29);
            dataGr[j][0]->SetMarkerColor(3);
            dataGr[j][0]->GetYaxis()->SetRangeUser(0,20000);                
            dataGr[j][0]->Draw("AP");
            dataGr[j][1]->SetMarkerStyle(24);
            dataGr[j][1]->SetMarkerColor(4);
            dataGr[j][1]->Draw("P same");

   TLegend *leg1 = new TLegend(0.18, 0.58 ,0.48, 0.88);
   leg1->AddEntry(dataGr[j][0], "before TB", "ap");
   leg1->AddEntry(dataGr[j][1], "after TB", "ap");
   leg1->Draw();

            int status=-1; TF1 *nf;
            if((vY[0]->at(0)+vY[0]->at(1))<0) nf = nfit(dataGr[j][0],mean[0],status);
            else if((vY[0]->at(0)+vY[0]->at(1))>0) nf = fit(dataGr[j][0],mean[0],status);
            else cout << "don't know what to do... breaking... " << endl;
            TF1 *nf1 = new TF1("nf1","[0]+[1]*exp(x*[2])+[3]*exp(x*[4])",0, 1500);
            for( int i=0; i<6; i++)
                nf1->SetParameter(i,nf->GetParameter(i));
            nf1->SetLineColor(3);
            nf1->Draw("same");
            ...
            status=-1; TF1 *ff;
            if((vY[1]->at(0)+vY[1]->at(1))>0) ff =fit(dataGr[j][1],mean[1],status);
            else if((vY[1]->at(0)+vY[1]->at(1))<0) ff = nfit(dataGr[j][1],mean[1],status);
            else cout << "don't know what to do... breaking... " << endl;
            ...
            //ofile << dataGr[j][2]->GetTitle() << "\t"<< xmax << "\t\t" << mean[2] << " " << rms[2]/TMath::Sqrt(n4mean) << "\t\t";
            //ofile << status << "\t\t" << ff->GetParameter(0)-nf->GetParameter(0) << " " << ff->GetParError(0) << "\t\t" <<  "\n";



        }else{
                std::cout << "can't open file " << (dirname1+filename+suffix[j]+TString(ext)) << " && " << (dirname2+filename+suffix[j]+TString(ext)) << std::endl;            
        };  
    }
/*
