...
TF1* fit(TGraphErrors* graph, double mean, int& status) {
  ...
  TF1* f1 = new TF1("f1", "[0]+[1]*exp(x*[2])+[3]*exp(x*[4])");
...
TGraphErrors* createGraph(
    std::vector<int>* troot, std::vector<double>* current, TString* title, double& mean, double& rms, bool& isErr) {
  int nX = troot->size();
  int nY = current->size();
  ...
  if ((nX == nY) && (nX >= n4mean)) {
    ...
    for (unsigned int i = firstPoints; i < nX - lastPoints; i++) {
      if (troot->at(i) < 0 or troot->at(i) > 10000) {
        continue;
      }
    ...
    std::cout << "different sizes of X and Y vectors, or too few points; don't know what to do... Data:  " << title
              << std::endl;
    isErr = false;
  };
  return g;
...
void test_plot(const char* dirnameChar,
               const char* fnameTChar = "ClosedCircuit_ShortCables_WithBox_0-300V_",
               const char* ext = ".txt") {
  //setTDRStyle();
  ...
  dirname += "/";
  TString suffix[12] = {"layer1_strips12",
                        "layer1_strips23",
                        "layer2_strips12",
                        "layer2_strips23",
                        "layer3_strips12",
  ...
  for (int j = 0; j < 12; j++) {
    ...
    if (myfile.is_open()) {
      ...
      std::cout << "name, mean, and normalized RMS: " << dataGr[j][1]->GetTitle() << "\t" << xmax << "\t\t" << mean[1]
                << " " << rms[1] / TMath::Sqrt(n4mean) << "\t" << std::endl;
      std::cout << "Fit status, final measured current, and error: " << status << "\t" << I300 - I0 << " "
                << ff->GetParError(0) << std::endl;
      // Title, I_max, I_meas, dI_fitStat, dI_fitSys, dI_tot, fit status
      ofile << dataGr[j][1]->GetTitle() << "\t" << xmax << "\t" << I300 - I0 << "\t" << ff->GetParError(0) << "\t"
            << I300 - I0 - (mean[1] - mean[0]) << "\t"
            << TMath::Sqrt(TMath::Power(I300 - I0 - (mean[1] - mean[0]), 2) + TMath::Power(ff->GetParError(0), 2) +
                           TMath::Power(nf->GetParError(0), 2))
            << '\t' << status << "\n";
