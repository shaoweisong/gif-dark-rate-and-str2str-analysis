...
void CSCDCCUnpacker::produce(edm::Event& e, const edm::EventSetup& c) {
  if (debug) std::cout << "CSCDCCUnpacker::produce()\n" ;
  ...
  if (debug) std::cout << std::dec << "CSCDCCUnpacker::produce():\tFEDNumbering: (MINCSCFEDID-MAXCSCFEDID) => defines cscFEDids " <<  FEDNumbering::MINCSCFEDID << " " << FEDNumbering::MAXCSCFEDID << std::endl;
  
  for (unsigned int id = FEDNumbering::MINCSCDDUFEDID; id <= FEDNumbering::MAXCSCDDUFEDID; ++id)  // loop over DDUs
  {
    cscFEDids.push_back(id);
  }
  if (debug) std::cout << std::dec << "CSCDCCUnpacker::produce():\tFEDNumbering: (MINCSCDDUFEDID-MAXCSCDDUFEDID) => defines cscFEDids " <<  FEDNumbering::MINCSCDDUFEDID << " " << FEDNumbering::MAXCSCDDUFEDID << std::endl;
  
  //if (debug)  std::cout << std::dec << "CSC FED size = " << cscFEDids.size() << std::endl;
  for (unsigned int i = 0; i < cscFEDids.size(); i++)  // loop over all CSC FEDs (DCCs and DDUs)
  {
    ...
    if (debug and length>0) std::cout << std::dec << "CSCDCCUnpacker::produce():\tFED id = " << id << "; is DDU_FED? "<< isDDU_FED << "; data length = " << length << std::endl;

    if (length >= 32)  ///if fed has data then unpack it
    {
      CSCDCCExaminer* examiner = nullptr;
      goodEvent = true;
	  if (debug) std::cout << "CSCDCCUnpacker::produce():\tuseExaminer = " << useExaminer << std::endl;
      if (useExaminer)  ///examine event for integrity
      {
        ...
        if (debug)
           std::cout << std::dec << "CSCDCCUnpacker::produce():\trunning Examiner: FED" << id << " ; size : " << fedData.size() << " good event: " << goodEvent << " examiner errors: "
              << std::dec << std::endl;
      ...
      if (goodEvent) {
        ...
        // H E R E my test
       /*
        if ( (ptrExaminer == nullptr) && isDDU_FED ) {
        	ptrExaminer = new CSCDCCExaminer();
        	ptrExaminer->modeDDU(true);
        }
        */
        
        if (debug) std::cout << "CSCDCCUnpacker::produce():\tbefore unpacking start : examiner == nullptr? " << (ptrExaminer == nullptr) << std::endl;
        ...
        {
          if (debug) std::cout << "CSCDCCUnpacker::produce():\tcreating CSCDDUEventData\n";
        ...
        {
          if (debug) std::cout << "CSCDCCUnpacker::produce():\tcreating CSCDCCEventData\n";
        ...
        } // end different FED readout modes
        ...
        for (unsigned int iDDU = 0; iDDU < dduData.size(); ++iDDU)  // loop over DDUs
        {
          ...
          if (dduData[iDDU].trailer().errorstat() & errorMask) {
            LogTrace("CSCDCCUnpacker|CSCRawToDigi")
                ...
                << dduData[iDDU].trailer().errorstat() << std::dec;
          ...
          if (debug && cscData.size() != 0) 
        	  std::cout << std::dec << "CSCDCCUnpacker::produce():\tFED" << id << " DDU Source ID: " << dduData[iDDU].header().source_id() << " DDU format version: " << dduData[iDDU].header().format_version() << std::endl;

          for (unsigned int iCSC = 0; iCSC < cscData.size(); ++iCSC)  // loop over CSCs
          {
        	  if(debug)
        	  	std::cout << std::dec << "CSCDCCUnpacker::produce():\tiCSC=" << iCSC  <<  " ; dmbHeader : original crateID, dmbID = " << cscData[iCSC].dmbHeader()->crateID() << "," 
        				  	<< cscData[iCSC].dmbHeader()->dmbID()<< ", format version : " <<  cscData[iCSC].dmbHeader()->format_version() << ", nalct = " << cscData[iCSC].dmbHeader()->nalct()
							<< std::endl;
            ...
            if (vmecrate == 1 && dmb == 2) {vmecrate = 1; dmb = 2;} // <===== GIF++
            ...
            if ((vmecrate >= 1) && (vmecrate <= 60) && (dmb >= 1) && (dmb <= 10) && (dmb != 6)) {
              ...
              if (debug) std::cout << std::dec << "CSCDCCUnpacker::produce():\tvmecrate, dmb = " << vmecrate << " " << dmb << "; layer as pcrate->detID(vmecrate, dmb, icfeb, ilayer)" << layer << "<-\n";
            } else {
            ...
            if (isDDU_FED) {
              ...
              // std::cout << "CSC " << layer << " -> " << id << ":" << dduid << ":" << vmecrate << ":" << dmb ;
                if (debug) std::cout << std::dec << "CSCDCCUnpacker::produce():\treadout FED/DDU ID=" << id << " expected ID=" << dduid << ", chamber " << layer
                << " vme= " << vmecrate << " dmb= " << dmb << std::endl;              
              }
              if (id != dduid) {
        ...
        if (examiner) {
          ...
          if (debug) {
            LogTrace("CSCDCCUnpacker|CSCRawToDigi")
                ...
                << (examiner->errors() & examinerMask) << std::dec;
          }
        }
